...
interface TideFetchResult {
  events: TideEvent[];
  seriesByDate: Record<string, TidePoint[]>;
  waveSeriesByDate: Record<string, WavePoint[]>;
}
...
const daysForecastHourly: Record<string, HourlyWeather[]> = {};
const daysForecastTides: Record<string, TideEvent[]> = {};
const daysForecastTideSeries: Record<string, TidePoint[]> = {};
const daysForecastWaveSeries: Record<string, WavePoint[]> = {};
const cityCoordinatesCache: Record<string, Coordinates> = {};
let seaWeatherEnabled = false;
let seaPanelCollapsed = false;
let unit = 'metric';

chrome.storage.sync.get(
  ['unit', 'seaPanelCollapsed'],
  (data: { unit?: string; seaPanelCollapsed?: boolean }) => {
  ...
  chrome.storage.sync.get(
    ...
    async (data: StorageData) => {
      ...
      try {
        ...
        const hourlyData: Record<string, HourlyWeather[]> = {};
        results.forEach((entry) => {
          const date = entry.dt_txt.split(' ')[0];
        ...
        const tideData: Record<string, TideEvent[]> = {};
        const tideSeriesData: Record<string, TidePoint[]> = {};
        const waveSeriesData: Record<string, WavePoint[]> = {};
          ...
          try {
            ...
            if (coords) {
              ...
              if (startDate && endDate) {
                const tideResult = await fetchTideEventsForRange(
                  coords,
                  startDate,
                  endDate,
                );
...
async function geocodeCity(
  ...
  const geocodeResults = (await response.json()) as GeocodeResult[];
  if (!Array.isArray(geocodeResults) || geocodeResults.length === 0) return null;
...
async function fetchTideEventsForRange(
  ...
  const marineData = (await response.json()) as MarineResponse;
  ...
  const seriesByDate: Record<string, TidePoint[]> = {};
  const waveSeriesByDate: Record<string, WavePoint[]> = {};

  for (let i = 0; i < heights.length; i += 1) {
    const height = heights[i];
    if (Number.isNaN(height)) continue;
    const [date, timePart = '00:00'] = times[i].split('T');
    if (!seriesByDate[date]) seriesByDate[date] = [];
  ...
  for (let i = 0; i < waveHeights.length; i += 1) {
    ...
    if (Number.isNaN(waveHeight)) continue;
    const timestamp = times[i];
    if (!timestamp) continue;
    const [date, timePart = '00:00'] = timestamp.split('T');
    if (!waveSeriesByDate[date]) waveSeriesByDate[date] = [];
...
function groupTideEventsByDate(
  events: TideEvent[],
): Record<string, TideEvent[]> {
  const grouped: Record<string, TideEvent[]> = {};
...
function formatWindSpeedForUnit(speed: number, currentUnit: string): string {
...
function createTideGraphElement(
  points: TidePoint[],
  events: TideEvent[],
): HTMLElement {
  ...
  const xAt = (idx: number): number =>
    chartLeft + (idx / Math.max(points.length - 1, 1)) * usableWidth;
  ...
  const findClosestIndexForHour = (hour: number): number => {
    let bestIndex = 0;
    let bestDistance = Number.POSITIVE_INFINITY;
    points.forEach((point, idx) => {
      const pointHour = parseInt(point.time.slice(0, 2), 10);
      if (Number.isNaN(pointHour)) return;
  ...
  const yAxisLabels = [
    ...
    .map(
      (label) =>
        `<text x="${(chartLeft - 6).toFixed(2)}" y="${(label.y + 3).toFixed(2)}" text-anchor="end" font-size="10" fill="#6f7782">${label.value.toFixed(2)} m</text>`,
    )
    .join('');
...
function createWaveTimelineElement(
  entries: WavePoint[],
): HTMLElement {
...
function hasSeaWeatherData(
  tideSeries: TidePoint[],
  tideEvents: TideEvent[],
  waveSeries: WavePoint[],
): boolean {
  const tideSeriesHasData = tideSeries.some((point) =>
    hasNonZeroNumber(point.height),
  );
  const tideEventsHaveData = tideEvents.some((event) =>
    hasNonZeroNumber(event.height),
  );
  const waveSeriesHasData = waveSeries.some(
    (point) =>
      hasNonZeroNumber(point.height) || hasNonZeroNumber(point.direction),
  );

  return tideSeriesHasData || tideEventsHaveData || waveSeriesHasData;
}

function showDetailPopup(date: string): void {
  ...
  let weatherContent: HTMLElement = content;
  ...
  if (seaWeatherEnabled) {
    ...
    const applySeaPanelState = (collapsed: boolean): void => {
